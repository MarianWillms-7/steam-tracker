...
function toggleVsPanel() {
    ...
    document.getElementById('vsOverlay').classList.toggle('active');
}
function closeVsPanel() {
    document.getElementById('vsPanel').classList.remove('active');
    document.getElementById('vsOverlay').classList.remove('active');
}
...
async function generateTagCloud() {
    ...
    for(let g of topGames) {
        if(g.id) {
            ...
            if(d && d.genres) d.genres.forEach(gen => tags[gen.description] = (tags[gen.description] || 0) + g.minutes);
        }
    }
    let sorted = Object.entries(tags).sort((a,b) => b[1] - a[1]);
    if(!sorted.length) { c.innerHTML = "Keine Daten"; return; }
    let max = sorted[0][1];
    c.innerHTML = sorted.map(([t, v]) => {
        let s = v > max * 0.7 ? "tag-l" : (v > max * 0.4 ? "tag-m" : "tag-s");
        return `<span class="tag-cloud-item ${s}">${t}</span>`;
    }).join('');
}
...
async function fetchGameDataInternal(id) {
    ...
    if(d) gameDataCache[id] = d; return d;
}
...
async function calcCompletion() {
    let el=document.getElementById('compRateVal'), sub=document.getElementById('compSub');
    if(!el) return; el.innerText="Lade..."; let entry=rawData.find(e=>e.name===currentUser), sid=entry?entry.steam_id:null;
    ...
    for(let g of games){
        ...
        try{
            ...
            if(txt&&!txt.includes("<error>")){
                let xml=new DOMParser().parseFromString(txt,"text/xml"), achs=xml.querySelectorAll('achievement');
                if(achs.length){ let d=0; achs.forEach(a=>{if(a.getAttribute('closed')==="1")d++;}); totalP+=(d/achs.length)*100; count++; }
            }
        }catch(e){}
...
function calculateRecap() {
    ...
    let log=rawData.filter(e=>e.name===currentUser); if(!log.length)return;
    let gs={}, da={}; log.forEach((e,i)=>{if(e.status!==0&&log[i+1]){let d=getDuration(e,log[i+1]); let g=e.game||"PC"; gs[g]=(gs[g]||0)+d; let k=new Date(e.time).toLocaleDateString('de-DE'); da[k]=(da[k]||0)+d;}});
    let tg=Object.entries(gs).sort((a,b)=>b[1]-a[1])[0], td=Object.entries(da).sort((a,b)=>b[1]-a[1])[0];
    document.getElementById('recapTopGame').innerText=tg?tg[0]:"-"; document.getElementById('recapTopDay').innerText=td?td[0]:"-";
    generateTagCloud();
}
