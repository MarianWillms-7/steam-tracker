...
// --- PRELOADING & SMART FETCH MIT ALTERSPRÜFUNG ---
...
async function fetchGameDataInternal(id) {
    ...
    // TRICK: Erst Deutsch probieren
    let targetUrl = `https://store.steampowered.com/api/appdetails?appids=${id}&cc=de&l=german`;
    let data = await tryFetch(targetUrl, id);
...
function processData() {
    ...
    document.getElementById('userAvatar').src = lastEntry.avatar || DEFAULT_AVATAR;
...
function calculateStats(userLog) {
    ...
    } else { document.getElementById('totalHours').innerText = (totalMinutes / 60).toFixed(1) + " h"; }
...
// --- NEW: LIBRARY WITH DROPDOWN ACHIEVEMENTS ---
...
// --- RENDER GAME DETAILS (HEADER) WITH AGE RATING ---
async function renderDetails() {
    ...
    if(gameData) {
        ...
        // FALLBACK
        document.getElementById('gameDetailsContent').innerHTML = `<div style='padding:30px; text-align:center; background:rgba(255,255,255,0.05); border-radius:12px;'><h3 style="color:#fff;">Spiel-Details</h3><p style="color:#94a3b8; font-size:0.9rem;">Steam-Shop Infos nicht verfügbar (evtl. Altersbeschränkung).</p><div class="playtime-display" style="font-size:1.2rem; margin:20px 0;">Deine Zeit: <strong style="color:var(--accent)">${myHours} Std</strong></div><a href="https://store.steampowered.com/app/${currentGameId}" target="_blank" class="steam-store-btn">Im Steam Shop öffnen ↗</a></div>`;
    }
}
